...
class NewsCell: UITableViewCell {
    ...
    private var cellHeight: CGFloat {
        return viewsLbl.downLeftCorner.y - avatarImageView.upLeftCorner.y + 2 * NEWSCONST.insets
    }
    ...
    private func setupAvatarImage(_ cellSize: CGRect) {
        ...
        let avatarOrigin = CGPoint(x: ceil(cellSize.origin.x + NEWSCONST.insets), y: ceil(cellSize.origin.y + NEWSCONST.insets))
        //avatarImageView.image = #imageLiteral(resourceName: "avatar-man")
        avatarImageView.contentMode = .scaleAspectFit
        avatarImageView.layer.cornerRadius = NEWSCONST.avatarSize / 2
        avatarImageView.clipsToBounds = true
        avatarImageView.frame = CGRect(origin: avatarOrigin, size: CGSize(width: NEWSCONST.avatarSize, height: NEWSCONST.avatarSize))
    ...
    private func setupTitleLbl(_ cellSize: CGRect) {
        // calculate frame
        let titleOriginX = ceil(2 * NEWSCONST.insets + avatarImageView.frame.width)
        let titleOriginY = ceil(avatarImageView.frame.origin.y + NEWSCONST.avatarSize / 2 - NEWSCONST.titleToDateInset / 2 - NEWSCONST.titleHeight)
        let titleSizeWidth = ceil(cellSize.width - (NEWSCONST.avatarSize + 3 * NEWSCONST.insets))
        let titleSizeHeight = ceil(NEWSCONST.titleHeight)
        // set frame
        ...
        titleLbl.font = NEWSCONST.titleFont
    }
  
    private func setupTimeLbl(_ cellSize: CGRect) {
        // calculate frame
        let timeOriginX = ceil(titleLbl.frame.origin.x)
        let timeOriginY = ceil(titleLbl.frame.origin.y + titleLbl.frame.size.height + NEWSCONST.titleToDateInset)
        ...
        timeLbl.font = NEWSCONST.dateFont
    }
    ...
    private func setupBodyLbl(_ cellSize: CGRect) {
        ...
        let bodyOriginY = ceil(avatarImageView.frame.origin.y + NEWSCONST.avatarSize + (haveNoText ? 0.0 : NEWSCONST.insets))
        let bodySizeWidth = ceil(cellSize.size.width - 2 * NEWSCONST.insets)
        let bodySizeHeight: CGFloat = haveNoText ? 0.0 :ceil(​getLabelSize(text: text, font: NEWSCONST.bodyFont, maxWidth: cellSize.size.width - 2 * NEWSCONST.insets).height)
        // set frame
        ...
        bodyLbl.font = NEWSCONST.dateFont
    }
    
    private func setupPictureImageView(_ cellSize: CGRect) {
        // calculate frame
        let pictureOriginX = ceil(bodyLbl.upLeftCorner.x)
        let pictureOriginY = ceil(bodyLbl.downLeftCorner.y + NEWSCONST.insets)
        let pictureSizeWidth = ceil(cellSize.size.width - 2 * NEWSCONST.insets)
        let pictureSizeHeight = containsImage ? ceil(NEWSCONST.pictureHeight) : 0.0
        // set frame
    ...
    private func setupLikesImageView(_ cellSize: CGRect) {
        ...
        let likesImgOriginY = ceil(pictureImageView.downLeftCorner.y + NEWSCONST.insets)
        let likeImgSizeWidth = ceil(NEWSCONST.iconSize)
        let likeImgSizeHeight = ceil(NEWSCONST.iconSize)
        // set frame
...
//        likesImageView.backgroundColor = #colorLiteral(red: 0.8549019694, green: 0.250980407, blue: 0.4784313738, alpha: 1)
    ...
    private func setupLikesLbl(_ cellSize: CGRect) {
        ...
        // calculate frame
        let likesOriginX = likesImageView.upRightCorner.x + NEWSCONST.iconsInset - 2.0
        let likesOriginY = likesImageView.upRightCorner.y
        let likesLblWidth = ​getLabelSize(text: text, font: NEWSCONST.bodyFont, maxWidth: 100.0).width
        ...
        // visual setup
        likesLbl.font = NEWSCONST.bodyFont
    ...
    private func setupRepostsImageView(_ cellSize: CGRect) {
        // calculate frame
        let originX = likesLbl.upRightCorner.x + NEWSCONST.iconsInset - 2.0
...
//        repostsImageView.backgroundColor = #colorLiteral(red: 0, green: 0.4784313725, blue: 1, alpha: 1)
    ...
    private func setupRepostsLbl(_ cellSize: CGRect) {
        ...
        // calculate frame
        let originX = repostsImageView.upRightCorner.x + NEWSCONST.iconsInset - 2.0
        let originY = repostsImageView.upRightCorner.y
        let repostsLblWidth = ​getLabelSize(text: text, font: NEWSCONST.bodyFont, maxWidth: 100.0).width
        ...
        // visual setup
        repostsLbl.font = NEWSCONST.bodyFont
    ...
    private func setupCommentsImageView(_ cellSize: CGRect) {
        // calculate frame
        let originX = repostsLbl.upRightCorner.x + NEWSCONST.iconsInset + 2.0
...
//        commentsImageView.backgroundColor = #colorLiteral(red: 0.4508578777, green: 0.9882974029, blue: 0.8376303315, alpha: 1)
    ...
    private func setupCommentLbl(_ cellSize: CGRect) {
        ...
        // calculate frame
        let originX = commentsImageView.upRightCorner.x + NEWSCONST.iconsInset
        let originY = commentsImageView.upRightCorner.y
        let commentsLblWidth = ​getLabelSize(text: text, font: NEWSCONST.bodyFont, maxWidth: 100.0).width
        ...
        // visual setup
        commentsLbl.font = NEWSCONST.bodyFont
    ...
    private func setupViewImageView(_ cellSize: CGRect) {
        // calculate frame
        let originX = commentsLbl.upRightCorner.x + NEWSCONST.iconsInset
...
//        viewsImageView.backgroundColor = #colorLiteral(red: 0.9994240403, green: 0.9855536819, blue: 0, alpha: 1)
    ...
    private func setupViewLbl(_ cellSize: CGRect) {
        ...
        // calculate frame
        let originX = viewsImageView.upRightCorner.x + NEWSCONST.iconsInset
        ...
        // visual setup
        viewsLbl.font = NEWSCONST.bodyFont
